# 👋 Hi, I'm Nhân Lê

🚀 **AI Engineer | Backend | MLOps Enthusiast**

Welcome to my **new GitHub profile**.  
This account marks a fresh start with cleaner structure, clearer focus, and newer projects.

---

## 🔁 Previous GitHub Account

Most of my **earlier work, repositories, and contributions** are available on my previous GitHub account:

👉 **Old account:** [https://github.com/USERNAME_CU](https://github.com/NahwngLe)

> This account is a **continuation / reference** of the old one.

---

## 🧠 Focus Areas

- 🤖 Artificial Intelligence & Machine Learning  
- 🧩 Backend Development (FastAPI, Node.js)
- 📦 MLOps (MLflow, Kubeflow, Docker, CI/CD)
- 📊 Monitoring & Observability (Prometheus, Grafana)
- 📄 Document AI (PDF processing, OCR, embeddings, LLMs)

---

## 📌 What you'll find here

- New projects & experiments
- Proof-of-concepts (POC)
- Clean, well-structured repositories
- Ongoing personal & academic work

---

## 📫 Contact

- GitHub (old): [https://github.com/USERNAME_CU](https://github.com/NahwngLe)
- GitHub (new): [https://github.com/USERNAME_MOI](https://github.com/nhanlequy12)

Thanks for stopping by! ⭐  
Feel free to explore or follow my work.
